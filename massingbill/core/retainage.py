...
from massingbill.core.money import Bp, Cents, apply_bp, bp, cents, percent_of


@dataclass(frozen=True)
class RetainageSpec:
    """The withholding policy, as six numbers.

    A plain value rather than the ORM row it is usually loaded from. The engine
    only ever reads these six fields, so taking the row would buy nothing and
    cost the core its independence -- and a caller that has a rate but no
    database (an estimator, another service, a vendored copy) could not use it.
    """

    mode: RetainageMode = RetainageMode.SPLIT
    rate_work_bp: int = 1000
    rate_stored_bp: int = 1000
    reduction_threshold_bp: int | None = None
    reduced_rate_bp: int | None = None
    statutory_cap_bp: int | None = None
...
def effective_rates(
    rule: RetainageSpec, *, completed_stored: Cents, contract_sum: Cents
) -> tuple[Bp, Bp]:
...
def compute(rule: RetainageSpec, bases: list[LineBasis], contract_sum: Cents) -> RetainageResult:
...
def exceeds_statutory_cap(rule: RetainageSpec, result: RetainageResult, base: Cents) -> bool:

...
| **P7 — Integrations & API** | `/api/massingbill/v1` + OpenAPI 3.1 + Python SDK, API keys, webhooks + delivery log + retry (to **any** subscriber URL), generic `OidcProvider`, `S3Storage`, Procore in/out, QBO, Sage export, SOV import. | Spec/route parity test green; webhook HMAC verifies against the reference implementation; Procore fixture round-trip passes; **suite still green with every optional adapter disabled**. |
